  ...
  return (
    <GlassCard className="h-full flex flex-col p-6 sm:p-8 relative group stagger-1">
      ...
      <div className="flex-1 flex items-center justify-center relative w-full h-full">
        {/* アニメーションで広がるコンテナ */}
        <div className="absolute bg-slate-900/80 rounded-lg overflow-hidden flex flex-col animate-expand-width animate-expand-height border-cyan-500/50 border-2">
          
          {/* ターミナル内部コンテンツ（枠が広がった後にフェードイン） */}
          <div className={`flex flex-col h-full w-full p-4 font-mono text-sm sm:text-base text-cyan-400 scanlines ${isBooted ? 'animate-fade-in-content' : 'opacity-0'}`}>
            {/* ヘッダーバー */}
            <div className="flex justify-between items-center border-b border-cyan-500/30 pb-2 mb-4 text-xs tracking-widest text-cyan-500/70">
            ...
            <div 
              ref={logContainerRef}
              className="flex-1 overflow-y-auto space-y-2 scrollbar-hide pb-4"
            >
              {logs.map((log, i) => (
                <div key={i} className={log.includes("#") ? "text-white font-bold mt-4" : "text-cyan-300/80 whitespace-pre-wrap pl-2 border-l-2 border-cyan-500/30 ml-1"}>
                  {log}
                </div>
              ))}
            </div>
            ...
            {/* 入力フォーム */}
            <form onSubmit={handleCommand} className="mt-auto flex items-center gap-2 pt-3 border-t border-cyan-500/30">
              <span className="text-pink-500 font-bold">root@sys:</span>
              <span className="text-cyan-600 font-bold">/{cwd.join("/")}</span>
              <span className="text-white">#</span>
              <input 
                type="text" 
                value={input}
                onChange={(e) => setInput(e.target.value)}
                className="bg-transparent border-none outline-none text-white w-full placeholder-cyan-800/50 focus:ring-0 p-0 ml-2"
